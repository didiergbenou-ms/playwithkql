...
def check_drafts_and_results(page):
    ...
    other_table = page.evaluate("""__kql.game.scene.getScene('Game').caseDef.tableMeta
      .find(t=>t.name!=='Heartbeat').name""")
    page.locator(".schema-table").get_by_role("button", name=other_table, exact=True).click()
    schema_draft = editor.input_value()
    assert schema_draft.startswith(other_table)
    close_terminal(page)
    editor = open_terminal(page)
...
def check_worker_recovery(page):
    ...
    expect(editor).to_have_value(first_starter(page))
    unchanged()
    editor.fill(page.evaluate("__kql.game.scene.getScene('Game').caseDef.challenges[0].solution"))
    run(page)
    expect(page.locator(".verdict-box.correct")).to_be_visible()
    page.locator(".celebrate").wait_for(state="detached")
    page.wait_for_function("workerStats.active===0")

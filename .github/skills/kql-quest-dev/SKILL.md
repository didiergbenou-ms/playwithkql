...
- Preserve phone portrait and landscape, safe areas, touch targets and the native keyboard. Do not disable zoom, autofocus the phone editor or restart a scene on rotation. Desktop retains 640x360/2x; mobile uses the pure `chooseViews` contract with a wider route overview above the portrait action view and a full-width landscape camera. Keep physics independent of canvas size. Resize may render one frozen frame while paused, then must sleep again; never advance scene timers or movement merely to repaint.
...
- `testViewport.ts` validates projection bounds and uniform scale; `testViewportBrowser.py` validates sleeping rotation, actual camera world views and parallax. `runBrowserChecks.py` keeps each browser suite's log and screenshots; CI uploads `browser-reports` even on failure. Scout local browser approval remains unresolved: use the remote Actions runner rather than retrying local Playwright or broadening file access.

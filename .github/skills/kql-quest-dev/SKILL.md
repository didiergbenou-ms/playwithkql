...
| Autocomplete viewport positioning | `src/ui/completionPlacement.ts`, `editorCompletion.css`, `KqlEditor.tsx` | `testCompletionPlacement.ts`, `testCompletionBrowser.py`, visual viewport and terminal Run footer |
| Desktop versus touch presentation | `src/ui/inputMode.ts`, `OptionsModal.tsx` | `TouchControls.tsx`, `App.tsx`, `TerminalModal.tsx`, `testInputMode.ts`, `testInputModeBrowser.py` |
...
| Compact phone HUD and menus | `src/ui/MobilePauseMenu.tsx`, `Hud.tsx`, `compactMenus.css` | `MainMenu.tsx`, `DifficultySelect.tsx`, `CharacterSelect.tsx`, `Briefing.tsx`, `testCompactUi.tsx`, `testCompactBrowser.py` |
...
| Adaptive mobile camera composition | `src/game/viewport.ts`, `config.ts`, `GameScene.ts` | `PhaserGame.tsx`, `mobile.css`, `testViewport.ts`, `testViewportBrowser.py` |
...
- Auto input mode follows the primary pointer, not `any-pointer`, `maxTouchPoints` or a sticky "touched once" flag. Keep the manual visit-scoped Auto/Keyboard/Touch override reachable on hybrids; mode changes must not reset runs, Phaser or query drafts. Never refocus the editor merely because the mode changed.
- Completion portals stay inside the modal focus trap, outside its clipping scroll container. Fit against actual visual-viewport offsets and exclude the touch action footer; flip above or overlay the visible editor rather than render below the phone keyboard. Scroll gestures must not accept an option. Import `editorCompletion.css` after responsive styles.
...
- Manual horizontal steering and Arcade drag must not fight each other. `horizontalDrag` keeps the existing braking for released/opposing inputs and uses zero drag while steering. Preserve character speed/jump/gravity limits; cover Sparky's standing-jump fixture as well as real running routes. Never "fix" a failed route by injecting player velocity or teleporting between its waypoints.
- Preserve phone portrait and landscape, safe areas, touch targets and the native keyboard. Do not disable zoom, autofocus the phone editor or restart a scene on rotation. Desktop retains 640x360/2x; mobile uses one camera with a 288x208 portrait field and full-width landscape framing. Do not restore the removed Route View. The portrait host collapses to its fitted height; orientation comes from the window, not that collapsed host's aspect. Keep physics independent of canvas size. Resize may render one frozen frame while paused, then must sleep again; never advance scene timers or movement merely to repaint.
- Phone HUD shows only identity, health, terminal progress and Menu. Secondary actions and full mission progress belong in `MobilePauseMenu`; do not re-expand them above the playfield. Selection choices and the Continue action should fit the first screen; long supporting details are disclosures. No-overflow checks alone missed the first mobile version's oversized menu. Enforce HUD height, playfield fit and spacing budgets in `testCompactBrowser.py`.
...
- Touch audio activation belongs on trusted pointer release/touchend or click/key events, not a one-shot touch pointerdown. Keep gesture recovery available after Safari interruptions. Handle `resume()` rejection explicitly; do not silently discard it or cache a pending non-gesture resume that prevents a later valid gesture. Sound-test cues wait for resume and are cancelled when their dialog closes. Do not override device Silent Mode or claim automated audio-node checks prove physical iPhone audibility.
...
- Preserve `Phaser.AUTO`, explicit `pixelArt`/`roundPixels`, desktop 640x360/2x and deferred engine loading. Mobile backing and camera zoom come from `viewport.ts`, independent of DPR, with one uniform CSS scale. Do not stretch a capped viewport to fill its parent. Canvas is a deprecated fallback, so any new WebGL-only feature needs an explicit fallback decision. Do not blanket-force `vertexRoundMode` on scaled objects.
...
- `testAudioBrowser.py` is the focused local touch-audio check. A stricter gesture fixture wraps a real AudioContext to cover blocked startup, later user-gesture resume, scheduled cues, mute settings and cancelled pending sound tests. A physical iPhone check is still needed for Silent Mode, output routing and audibility.
...
- `testViewport.ts` validates projection bounds and uniform scale; `testViewportBrowser.py` validates sleeping rotation, actual camera world views and parallax. `testWorldAndTerminalBrowser.py` is the focused local camera/pane-scroll check; run it with WebGL and `--canvas`. Prefer affected checks for small changes rather than manually rerunning every remote suite. Local Scout testing works via saved scripts with explicit executable paths; detached shell quoting may require a saved PowerShell launcher. If permissions block a run, stop and surface it instead of leaving an approval pending or broadening file access.
- CI's optional `browser_scope` can isolate `mobile-ui`, `mobile-play` or the short `route` diagnostic. Only `all` is release validation; partial reports are labelled and deliberately omit the release artifact. Never promote a diagnostic-only pass to the Azure test site.

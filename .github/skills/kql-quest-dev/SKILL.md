...
- CI's optional `browser_scope` can isolate `mobile-ui` or `mobile-play` diagnostics. Only `all` is release validation; partial reports are labelled and deliberately omit the release artifact. Never promote a diagnostic-only pass to the Azure test site.

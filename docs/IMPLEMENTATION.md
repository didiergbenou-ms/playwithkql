...
Deployed by `.github/workflows/pages.yml` on every push to `main`: the same
gates as CI (typecheck, tests, UI tests, fuzz) must pass before anything is
published, so a broken build cannot reach the site.

**One-time setup**, by someone with admin on this repository:
Settings → Pages → Build and deployment → Source: **GitHub Actions**.
Until that is set, the workflow succeeds through build and fails at deploy.
...
domain root is not a passing build. The workflow derives it from the repository
name rather than hardcoding it, so a rename or a fork cannot silently break it.

The workflow also writes `.nojekyll` (without it Pages runs the output through
Jekyll, which silently drops paths beginning with an underscore) and copies
`index.html` to `404.html` so a refresh on an unknown path lands in the game
rather than on GitHub's 404.

> Earlier history note: while the source was in a private repository, the build
> was published to a *separate* public repo by a `scripts/deploy.mjs` script,
> because Pages on a private repo is a paid feature and the published site is
> access-restricted to people with repo access. That split is unnecessary here —
> this repository is public — so the script was removed in favour of deploying
> Pages directly.
